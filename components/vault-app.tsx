...
function FusionView() {
  ...
  const sourceIdeas=ideas.filter((idea)=>selectedIds.includes(idea.id));
  const sourceA=sourceIdeas[0]?.title||'tactile proof'; const sourceB=sourceIdeas[1]?.title||'founder confession';
  const pack={hook:`I combined “${sourceA}” with “${sourceB}”—and the result changed the brief.`,script:`Open on five micro-proofs from ${sourceA}. Break the expected reveal with the central tension from ${sourceB}. Resolve with one executable lesson and a save-worthy prompt.`,storyboard:'01 / tactile cold open · 02 / pattern interrupt · 03 / founder tension · 04 / proof sequence · 05 / quiet reveal · 06 / save CTA',shots:'Macro detail · overhead process · direct-to-camera confession · single-take reveal · clean end card',thumbnail:`Split-frame proof vs. confession. Lime signal ring around the key object. Four-word headline: “PROOF BEFORE AMBITION.”`,thumbnailPrompt:'Brutalist editorial thumbnail, black field, tactile product macro, founder silhouette, acid-lime signal ring, high contrast, premium cinematic grain, bold condensed headline',caption:`The strongest ideas rarely live in one folder. This concept connects ${sourceA} with ${sourceB}—proof first, ambition second.`,hashtags:'#creativeintelligence #storytelling #contentstrategy #buildinpublic #vaultai',audio:'Low-BPM tactile foley with a rising analog pulse at the reveal.',schedule:'Tuesday 11:40 / Thursday 18:10 / Sunday 09:20 — test the hook before scaling.',virality:'87 / 100 · High save intent · Medium comment volatility',angle:'Alternative: reverse the structure—open with confession, then earn trust through proof.'};
  const generate=()=>{setPhase("running");setTimeout(()=>setPhase("done"),1800)};
  const downloadPack=()=>{const content=`VAULT//AI PRODUCTION PACK\n\nSOURCE DNA\n${sourceIdeas.map((idea,index)=>`${index+1}. ${idea.title} — ${idea.insight}`).join('\n')}\n\nVIRAL HOOK\n${pack.hook}\n\nSCRIPT\n${pack.script}\n\nSTORYBOARD\n${pack.storyboard}\n\nSHOT LIST\n${pack.shots}\n\nTHUMBNAIL CONCEPT\n${pack.thumbnail}\n\nTHUMBNAIL AI PROMPT\n${pack.thumbnailPrompt}\n\nCAPTION\n${pack.caption}\n\nHASHTAGS\n${pack.hashtags}\n\nTRENDING AUDIO\n${pack.audio}\n\nPUBLISHING SCHEDULE\n${pack.schedule}\n\nVIRALITY PREDICTION\n${pack.virality}\n\nALTERNATIVE ANGLE\n${pack.angle}`;const url=URL.createObjectURL(new Blob([content],{type:'text/plain'}));const link=document.createElement('a');link.href=url;link.download='vault-ai-production-pack.txt';link.click();URL.revokeObjectURL(url);setExported(true)};
  return <><Header index="04" title="FUSION / LAB" copy="Combine distant signals. Generate something that did not exist before." />
    ...
    <section className="fusion-result"><div className="panel-heading"><span>GENERATED OUTPUT / SOURCE-AWARE</span><b>{phase==='done'?'READY':'WAITING'}</b></div>{phase==='done'?<motion.div initial={{opacity:0}} animate={{opacity:1}}><span className="tag">PRIMARY CONCEPT · 87 VIRALITY</span><h3>Show the evidence before the ambition.</h3><p>This output explicitly fuses <b>{sourceA}</b> with <b>{sourceB}</b>. Every production choice traces back to that source DNA.</p><dl><dt>VIRAL HOOK</dt><dd>{pack.hook}</dd><dt>SCRIPT</dt><dd>{pack.script}</dd><dt>STORYBOARD</dt><dd>{pack.storyboard}</dd><dt>SHOT LIST</dt><dd>{pack.shots}</dd><dt>THUMBNAIL CONCEPT</dt><dd>{pack.thumbnail}</dd><dt>THUMBNAIL AI PROMPT</dt><dd>{pack.thumbnailPrompt}</dd><dt>CAPTION + HASHTAGS</dt><dd>{pack.caption}<br/>{pack.hashtags}</dd><dt>TRENDING AUDIO</dt><dd>{pack.audio}</dd><dt>PUBLISHING SCHEDULE</dt><dd>{pack.schedule}</dd><dt>ALTERNATIVE ANGLE</dt><dd>{pack.angle}</dd></dl><button className="outline-btn" onClick={downloadPack}>{exported?'PACK DOWNLOADED':'DOWNLOAD COMPLETE PRODUCTION PACK'}</button></motion.div>:<EmptyState title="AWAITING FUSION" copy="Select at least two source ideas to unlock source-aware hooks, scripts, storyboards, shot lists, thumbnails and publishing plans."/>}</section></div></>;
}
...
export function VaultApp({initialView}:{initialView:ViewName}) {
  ...
  useEffect(()=>{
    ...
    try{const saved=localStorage.getItem('vault-ai:providers');if(saved)useVaultStore.getState().setEnabledProviders(JSON.parse(saved) as Record<string,boolean>);const dismissed=JSON.parse(localStorage.getItem('vault-ai:dismissed-notifications')||'[]') as string[];dismissed.forEach(dismissNotification)}catch{}
