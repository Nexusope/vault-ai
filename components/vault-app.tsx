...
function GalaxyView() {
  const ideas=useVaultStore((state)=>state.ideas);
  return <IdeaGalaxyWorkspace ideas={ideas}/>;
}

function buildFusionPack(sourceIdeas: Idea[], constraint: string) {
  const primary=sourceIdeas[0]; const contrast=sourceIdeas[1]; const supporting=sourceIdeas.slice(2);
  const sourceA=primary?.title||"tactile proof"; const sourceB=contrast?.title||"founder confession";
  const openingA=primary?.hook||primary?.insight||sourceA; const openingB=contrast?.hook||contrast?.insight||sourceB;
  const tags=[...new Set(sourceIdeas.flatMap((idea)=>idea.tags))];
  const averageTrend=Math.round(sourceIdeas.reduce((sum,idea)=>sum+idea.trend,0)/Math.max(sourceIdeas.length,1));
  const supportLine=supporting.length?`Use ${supporting.map((idea)=>idea.title).join(", ")} as supporting proof.`:"Resolve the tension with one specific proof point.";
  return {
    conceptTitle:`${primary?.category||"Proof"} meets ${contrast?.category||"confession"}.`,
    hook:`${openingA} Then interrupt the expected answer with: ${openingB}`,
    script:`Open on the strongest visual from “${sourceA}”. State its promise in one sentence. At second three, introduce the opposing tension from “${sourceB}”. ${supportLine} Close by turning the contrast into one action worth saving.`,
    storyboard:`01 / visual proof from ${sourceA} · 02 / opening promise · 03 / pattern interrupt from ${sourceB} · 04 / supporting evidence · 05 / practical resolution · 06 / save CTA`,
    shots:"Tactile macro · creator close-up · on-screen contrast card · overhead process · proof sequence · clean end frame",
    thumbnail:`Split-frame “${sourceA}” against “${sourceB}”. Put the contradiction, not the topic, in a four-word headline.`,
    thumbnailPrompt:`Brutalist editorial thumbnail, black field, acid-lime signal mark, visual tension between ${sourceA} and ${sourceB}, high contrast, tactile detail, cinematic grain, bold condensed four-word headline`,
    caption:`Two useful ideas become more valuable when they disagree. “${sourceA}” supplies the proof; “${sourceB}” supplies the tension. Constraint: ${constraint}.`,
    hashtags:(tags.length?tags.slice(0,5):["creativeintelligence","storytelling","contentstrategy"]).map((tag)=>`#${tag.replace(/\s+/g,"")}`).join(" "),
    audio:"Begin with isolated tactile sound, introduce a low analog pulse at the pattern interrupt, then remove the beat for the final line.",
    schedule:"Publish the first hook test Tuesday at 11:40. Test the alternate opening Thursday at 18:10. Scale the winner Sunday at 09:20.",
    virality:`${Math.min(96,averageTrend+2)} / 100 · ${averageTrend>=85?"High":"Medium"} save intent · Source-weighted estimate`,
    angle:`Alternative: open with “${sourceB}” as the confession, then earn trust by revealing “${sourceA}” as proof.`,
  };
}

function FusionView() {
  const {selectedIds,setSelectedIds,clearSelected,ideas}=useVaultStore(); const enabledProviders=useVaultStore((state)=>state.enabledProviders);
  const [phase,setPhase]=useState<"idle"|"running"|"done">("idle"); const [exported,setExported]=useState(false); const [constraint,setConstraint]=useState("Unexpected but executable"); const [routeNote,setRouteNote]=useState(""); const [routeLabel,setRouteLabel]=useState("LOCAL READY"); const [selectionNote,setSelectionNote]=useState("");
  const sourceIdeas=ideas.filter((idea)=>selectedIds.includes(idea.id)).slice(0,5);
  const rankedIdeas=useMemo(()=>[...ideas].sort((a,b)=>Number(Boolean(ideaThumbnail(b)))-Number(Boolean(ideaThumbnail(a)))||b.trend-a.trend),[ideas]);
  const pack=useMemo(()=>buildFusionPack(sourceIdeas,constraint),[sourceIdeas,constraint]);
  const resetFusion=()=>{setPhase("idle");setExported(false);setRouteNote("");setRouteLabel("LOCAL READY")};
  const handleSource=(id:string)=>{if(selectedIds.includes(id)){setSelectedIds(selectedIds.filter((item)=>item!==id));setSelectionNote("");resetFusion();return}if(sourceIdeas.length>=5){setSelectionNote("MAXIMUM 5 SOURCES");return}setSelectedIds([...selectedIds,id]);setSelectionNote("");resetFusion()};
  const autoPick=()=>{setSelectedIds(rankedIdeas.slice(0,2).map((idea)=>idea.id));setSelectionNote("2 HIGH-SIGNAL SOURCES READY");resetFusion()};
  const generate=async()=>{if(sourceIdeas.length<2){setSelectionNote(`SELECT ${2-sourceIdeas.length} MORE SOURCE${sourceIdeas.length===1?"":"S"}`);return}setPhase("running");setRouteNote("");setSelectionNote("");const providers=Object.entries(enabledProviders).filter(([,on])=>on).map(([name])=>name);const prompt=`Create one production-ready creative fusion from these saved ideas. Respect the constraint: ${constraint}. Explain the unexpected connection and give a sharper hook or execution insight. Do not repeat the source notes verbatim.\n\n${sourceIdeas.map((idea,index)=>`SOURCE ${index+1}: ${idea.title}\nHOOK: ${idea.hook||idea.title}\nINSIGHT: ${idea.insight}\nTAGS: ${idea.tags.join(", ")}`).join("\n\n")}`;try{const response=await fetch('/api/ai',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({prompt,providers}),signal:AbortSignal.timeout(12000)});const data=await response.json() as {content?:string;provider?:string};if(response.ok&&data.content&&data.provider!=="local"){setRouteNote(data.content);setRouteLabel(`${data.provider?.toUpperCase()||"AI"} ENRICHED`)}else{setRouteNote("The live provider route is unavailable, so Vault completed this fusion with the deterministic local engine. The full production pack remains usable.");setRouteLabel("LOCAL FALLBACK")}}catch{setRouteNote("The provider route could not be reached, so Vault completed this fusion locally. No source work was lost.");setRouteLabel("LOCAL FALLBACK")}finally{setPhase("done")}};
  const downloadPack=()=>{const content=`VAULT//AI PRODUCTION PACK\n\nSOURCE DNA\n${sourceIdeas.map((idea,index)=>`${index+1}. ${idea.title} — ${idea.insight}`).join('\n')}\n\nCREATIVE CONSTRAINT\n${constraint}\n\nCONCEPT\n${pack.conceptTitle}\n\nVIRAL HOOK\n${pack.hook}\n\nSCRIPT\n${pack.script}\n\nSTORYBOARD\n${pack.storyboard}\n\nSHOT LIST\n${pack.shots}\n\nTHUMBNAIL CONCEPT\n${pack.thumbnail}\n\nTHUMBNAIL AI PROMPT\n${pack.thumbnailPrompt}\n\nCAPTION\n${pack.caption}\n\nHASHTAGS\n${pack.hashtags}\n\nTRENDING AUDIO\n${pack.audio}\n\nPUBLISHING SCHEDULE\n${pack.schedule}\n\nVIRALITY PREDICTION\n${pack.virality}\n\nALTERNATIVE ANGLE\n${pack.angle}\n\nAI SYNTHESIS\n${routeNote}`;const url=URL.createObjectURL(new Blob([content],{type:'text/plain'}));const link=document.createElement('a');link.href=url;link.download='vault-ai-production-pack.txt';link.click();URL.revokeObjectURL(url);setExported(true)};
  return <><Header index="04" title="FUSION / LAB" copy="Combine distant signals. Generate something that did not exist before." />
    <div className="fusion-layout"><section><div className="panel-heading"><span>SOURCE MATERIAL / SELECT 2–5</span><b>{sourceIdeas.length} SELECTED</b></div><div className="fusion-source-actions"><button type="button" onClick={autoPick}>AUTO-PICK 2 HIGH SIGNALS</button><span aria-live="polite">{selectionNote||`${Math.max(0,2-sourceIdeas.length)} MORE REQUIRED`}</span></div><div className="fusion-source">{rankedIdeas.map(i=><button key={i.id} onClick={()=>handleSource(i.id)} className={selectedIds.includes(i.id)?'selected':''} aria-pressed={selectedIds.includes(i.id)}><span style={{background:i.accent}}/><div><b>{i.title}</b><small>{i.category} · {i.trend} signal</small></div>{selectedIds.includes(i.id)?<Check/>:<Plus/>}</button>)}</div></section>
    <section className="fusion-core"><div className={`fusion-orb ${phase}`}><Blocks size={44}/></div><span>CREATIVE CONSTRAINT</span><select aria-label="Fusion creative constraint" value={constraint} onChange={(event)=>{setConstraint(event.target.value);resetFusion()}}><option>Unexpected but executable</option><option>High-retention short form</option><option>Founder-led narrative</option></select><button className="primary-btn" disabled={phase==='running'} onClick={generate}>{phase==='running'?'SYNTHESIZING...':sourceIdeas.length<2?`SELECT ${2-sourceIdeas.length} MORE`:'INITIATE FUSION'} <Zap size={16}/></button><button className="text-btn" onClick={()=>{clearSelected();setSelectionNote("");resetFusion()}}>CLEAR INPUTS</button></section>
    <section className="fusion-result"><div className="panel-heading"><span>GENERATED OUTPUT / SOURCE-AWARE</span><b>{phase==='done'?routeLabel:phase==='running'?'ROUTING':'WAITING'}</b></div>{phase==='done'?<motion.div initial={{opacity:0}} animate={{opacity:1}}><span className="tag">PRIMARY CONCEPT · {pack.virality.split(' ')[0]} VIRALITY</span><h3>{pack.conceptTitle}</h3><p>This output fuses <b>{sourceIdeas[0]?.title}</b> with <b>{sourceIdeas[1]?.title}</b> under the constraint <b>{constraint}</b>.</p><dl>{routeNote&&<><dt>AI SYNTHESIS</dt><dd>{routeNote}</dd></>}<dt>VIRAL HOOK</dt><dd>{pack.hook}</dd><dt>SCRIPT</dt><dd>{pack.script}</dd><dt>STORYBOARD</dt><dd>{pack.storyboard}</dd><dt>SHOT LIST</dt><dd>{pack.shots}</dd><dt>THUMBNAIL CONCEPT</dt><dd>{pack.thumbnail}</dd><dt>THUMBNAIL AI PROMPT</dt><dd>{pack.thumbnailPrompt}</dd><dt>CAPTION + HASHTAGS</dt><dd>{pack.caption}<br/>{pack.hashtags}</dd><dt>TRENDING AUDIO</dt><dd>{pack.audio}</dd><dt>PUBLISHING SCHEDULE</dt><dd>{pack.schedule}</dd><dt>ALTERNATIVE ANGLE</dt><dd>{pack.angle}</dd></dl><button className="outline-btn" onClick={downloadPack}>{exported?'PACK DOWNLOADED':'DOWNLOAD COMPLETE PRODUCTION PACK'}</button></motion.div>:<EmptyState title={phase==='running'?"FUSING SOURCE DNA":"AWAITING FUSION"} copy={phase==='running'?"Vault is routing the connection through the strongest available intelligence provider.":"Select two to five source ideas—or use Auto-pick—to generate a complete source-aware production pack."}/>}</section></div></>;
}

function AssistantView() {
  const enabledProviders=useVaultStore((state)=>state.enabledProviders); const ideas=useVaultStore((state)=>state.ideas); const [messages,setMessages]=useState([{role:'assistant',text:'Vault graph online. I can search your ideas, surface cross-cluster patterns, explain relationships, or build a production-ready concept.'}]); const [input,setInput]=useState(''); const [busy,setBusy]=useState(false);
  const send=async()=>{if(!input.trim())return;const q=input;setInput('');setMessages(m=>[...m,{role:'user',text:q}]);setBusy(true);try{const providers=Object.entries(enabledProviders).filter(([,on])=>on).map(([name])=>name);const graphContext=galaxyContextForAI(ideas);const prompt=`Reason over this Vault AI knowledge graph. Use the supplied nodes and relationships as evidence; distinguish graph evidence from suggestions.\n\nGRAPH_CONTEXT=${graphContext}\n\nUSER_REQUEST=${q}`;const response=await fetch('/api/ai',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({prompt,providers})});const data=await response.json() as {content?:string};setMessages(m=>[...m,{role:'assistant',text:data.content||'The provider router did not return usable content.'}])}catch{setMessages(m=>[...m,{role:'assistant',text:'The provider router is temporarily unreachable. Your prompt is preserved; retry when connectivity returns.'}])}finally{setBusy(false)}};
...
function CaptureModal({close}:{close:()=>void}){
  const [form,setForm]=useState({title:'',sourceUrl:'',summary:'',creator:'unknown',hook:''}); const [status,setStatus]=useState<'idle'|'saving'|'saved'|'error'>('idle');
  ...
  const submit=async(e:React.FormEvent)=>{e.preventDefault();setStatus('saving');try{let uploadedMediaUrl:string|undefined;let thumbnailUrl:string|undefined;let mediaType: string|undefined;if(file){const payload=new FormData();payload.set('file',file);const upload=await fetch('/api/media',{method:'POST',body:payload});if(!upload.ok)throw new Error('upload failed');const uploaded=await upload.json() as {mediaUrl:string;kind:string};uploadedMediaUrl=uploaded.mediaUrl;mediaType=uploaded.kind;if(uploaded.kind==='image')thumbnailUrl=uploaded.mediaUrl}const sourceUrl=form.sourceUrl||uploadedMediaUrl;const response=await fetch('/api/ideas',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({...form,sourceUrl:sourceUrl||undefined,thumbnailUrl,hook:form.hook||undefined,mediaType,tags:['captured']})});if(!response.ok)throw new Error('save failed');const data=await response.json() as {idea:StoredIdea};hydrateIdeas([data.idea]);setStatus('saved');setTimeout(close,800)}catch{setStatus('error')}};
...
export function VaultApp({initialView}:{initialView:ViewName}) {
  ...
  useEffect(()=>{
    let cancelled=false;
    const loadIdeas=async()=>{let offset=0;const accumulated:StoredIdea[]=[];while(!cancelled){const response=await fetch(`/api/ideas?limit=250&offset=${offset}`);if(!response.ok)break;const data=await response.json() as {ideas:StoredIdea[];pagination?:{hasMore:boolean;nextOffset:number|null}};accumulated.push(...data.ideas);hydrateIdeas(accumulated);if(!data.pagination?.hasMore||data.pagination.nextOffset===null)break;offset=data.pagination.nextOffset;await new Promise((resolve)=>setTimeout(resolve,0));}};
    void loadIdeas().catch(()=>undefined);
    try{const saved=localStorage.getItem('vault-ai:providers');if(saved)useVaultStore.getState().setEnabledProviders(JSON.parse(saved) as Record<string,boolean>);const selected=JSON.parse(localStorage.getItem('vault-ai:selected')||'[]') as string[];useVaultStore.getState().setSelectedIds(selected);const dismissed=JSON.parse(localStorage.getItem('vault-ai:dismissed-notifications')||'[]') as string[];dismissed.forEach(dismissNotification)}catch{}
    return()=>{cancelled=true};
  },[dismissNotification,hydrateIdeas]);
