...
function LibraryView() {
  const ideas=useVaultStore((state)=>state.ideas); const [mode,setMode]=useState<"grid"|"list">("grid"); const [query,setQuery]=useState(()=>typeof window==='undefined'?'':new URLSearchParams(window.location.search).get('q')||''); const [source,setSource]=useState('ALL');
...
export function VaultApp({initialView}:{initialView:ViewName}) {
  ...
  useEffect(()=>{const onKey=(e:KeyboardEvent)=>{if((e.ctrlKey||e.metaKey)&&e.key==='k'){e.preventDefault();setCommand(v=>{if(v)setTimeout(()=>commandTrigger.current?.focus(),0);return !v})} if(e.key==='Escape'){setCommand(false);setMobileOpen(false);setTimeout(()=>commandTrigger.current?.focus(),0)} if(e.key==='Tab'&&command&&commandDialog.current){const focusable=Array.from(commandDialog.current.querySelectorAll<HTMLElement>('input,a,button')).filter((element)=>!element.hasAttribute('disabled'));if(!focusable.length)return;const first=focusable[0],last=focusable[focusable.length-1];if(e.shiftKey&&document.activeElement===first){e.preventDefault();last.focus()}else if(!e.shiftKey&&document.activeElement===last){e.preventDefault();first.focus()}}};window.addEventListener('keydown',onKey);return()=>window.removeEventListener('keydown',onKey)},[command]);
