...
function CaptureModal({close}:{close:()=>void}){
  ...
  const submit=async(e:React.FormEvent)=>{e.preventDefault();setStatus('saving');try{let sourceUrl=form.sourceUrl;let mediaType: string|undefined;if(file){const payload=new FormData();payload.set('file',file);const upload=await fetch('/api/media',{method:'POST',body:payload});if(!upload.ok)throw new Error('upload failed');const uploaded=await upload.json() as {mediaUrl:string;kind:string};sourceUrl=uploaded.mediaUrl;mediaType=uploaded.kind}const response=await fetch('/api/ideas',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({...form,sourceUrl:sourceUrl||undefined,mediaType,tags:['captured']})});if(!response.ok)throw new Error('save failed');const data=await response.json() as {idea:StoredIdea};hydrateIdeas([data.idea]);setStatus('saved');setTimeout(close,800)}catch{setStatus('error')}};
  return <div className="modal-overlay" role="presentation" onMouseDown={close}><motion.form ref={dialogRef} role="dialog" aria-modal="true" aria-labelledby="capture-title" initial={{opacity:0,scale:.97}} animate={{opacity:1,scale:1}} onMouseDown={e=>e.stopPropagation()} onSubmit={submit}><div className="modal-head"><div><span>{"// NEW SIGNAL"}</span><h2 id="capture-title">Capture an idea.</h2></div><button type="button" aria-label="Close capture dialog" onClick={close}><X/></button></div><label>TITLE<input autoFocus required minLength={2} value={form.title} onChange={e=>setForm({...form,title:e.target.value})} placeholder="What should you remember?"/></label><label>CONTENT SOURCE URL<input type="url" value={form.sourceUrl} onChange={e=>setForm({...form,sourceUrl:e.target.value})} placeholder="Instagram, TikTok, YouTube, X, Reddit, Pinterest, Threads or any website"/></label><label>OR UPLOAD PDF / IMAGE / VOICE NOTE<input type="file" accept="application/pdf,image/png,image/jpeg,image/webp,image/gif,audio/mpeg,audio/mp4,audio/wav,audio/webm,audio/ogg" onChange={e=>setFile(e.target.files?.[0])}/></label><label>CREATOR<input value={form.creator} onChange={e=>setForm({...form,creator:e.target.value})} placeholder="@creator"/></label><label>CONTEXT<textarea value={form.summary} onChange={e=>setForm({...form,summary:e.target.value})} placeholder="Why is this signal valuable?"/></label><div className="modal-actions"><span aria-live="polite">{status==='error'?'CAPTURE OR UPLOAD FAILED':status==='saved'?'CAPTURED + INDEXED':file?`${file.name} READY`:''}</span><button type="submit" className="primary-btn" disabled={status==='saving'}>{status==='saving'?'INDEXING...':'SAVE TO VAULT'}<ChevronRight/></button></div></motion.form></div>;
}
