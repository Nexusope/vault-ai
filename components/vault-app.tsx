...
function CaptureModal({close}:{close:()=>void}){
  ...
  const submit=async(e:React.FormEvent)=>{e.preventDefault();setStatus('saving');try{const response=await fetch('/api/ideas',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({...form,tags:['captured']})});if(!response.ok)throw new Error('save failed');const data=await response.json() as {idea:{id:string;title:string;creator:string;summary:string;source:string;sourceUrl?:string|null;tags:string}};hydrateIdeas([data.idea]);setStatus('saved');setTimeout(close,800)}catch{setStatus('error')}};
