...
test("finished Vault AI source replaces every starter surface", async () => {
  ...
  assert.match(layout, /og-armor\.png/);
...
test("ships complete blueprint, agents, skills, prompts, and project asset", async () => {
  const [docs, agents, skills, prompts, image] = await Promise.all([
    ...
    stat(new URL("../public/og-armor.png", import.meta.url)),
  ]);
...
test("saved-post surfaces preserve media, creator, hook, and save-time context", async () => {
  ...
  assert.match(component, /SAVED JUST NOW/);
  assert.match(component, /Number\(Boolean\(ideaThumbnail\(b\)\)\)/);
  assert.match(galaxy, /inspectorHook/);
  assert.match(galaxy, /SAVED JUST NOW/);
